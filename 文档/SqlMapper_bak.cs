...
this IDbTransaction transaction, string sql, dynamic param = null, int? commandTimeout = null, CommandType? commandType = null
        ...
        {
            return Execute(transaction.Connection, sql, param, transaction, commandTimeout, commandType);
        }
...
#pragma warning restore 612, 618
                ...
                else
                {
                    ...
                    if (s != null && s.Length + 1 > 4000 && p.GetType().Name == "SqlCeParameter")
                    {
                        var set = FastInvoke.GetMethodInvoker(p.GetType().GetProperty("SqlDbType").GetSetMethod());
                        set.Invoke(p, System.Data.SqlDbType.NText);
                        p.Size = Math.Max(s.Length + 1, 4000);
                    }
                    if (add)
                    {
