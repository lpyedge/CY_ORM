...
namespace Dapper.Sql
{
    ...
    public class SqlGeneratorImpl : ISqlGenerator
    {
        ...
        {
			var columns = classMap.Properties.Where(p => !(p.Ignored || p.IsReadOnly || p.KeyType == KeyType.Identity || !p.PropertyInfo.CanWrite));
            if (!columns.Any())
            {
        ...
        public virtual string BuildSelectColumns(IClassMapper classMap)
        {
            var columns = classMap.Properties
                .Where(p => !p.Ignored && p.PropertyInfo.CanWrite)
