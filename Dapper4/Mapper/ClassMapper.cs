        ...
        protected virtual void AutoMap(Func<Type, PropertyInfo, bool> canMap)
        {
            ...
            foreach (var propertyInfo in type.GetProperties().Where(p => p.CanRead && p.CanWrite))
            {
                ...
                if (!hasDefinedKey)
                {
                    ...
                    //如果映射不到主键，属性名为name的则为主键 customedit
                    if (keyMap == null && string.Equals(map.PropertyInfo.Name, "name", StringComparison.InvariantCultureIgnoreCase))
                    {
                        keyMap = map;
                    }

                    //如果映射不到主键，属性名为name结尾的则为主键 customedit
                    if (keyMap == null && map.PropertyInfo.Name.EndsWith("name", true, CultureInfo.InvariantCulture))
                    {
                        keyMap = map;
                    }
                }
            ...
            //如果映射不到主键，默认第一个属性为主键 customedit
            if (keyMap == null)
            {
                keyMap = (PropertyMap)Properties.FirstOrDefault();
            }
