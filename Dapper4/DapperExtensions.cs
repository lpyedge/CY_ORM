...
namespace Dapper
{
    public static class DapperExtensions
    {
        ...
        /// Configure DapperExtensions extension methods.
        /// </summary>
        public static void Configure<T>() where T : ISqlDialect
        {
            Configure(new DapperExtensionsConfiguration(typeof(AutoClassMapper<>), new List<Assembly>(), (ISqlDialect)Assembly.GetExecutingAssembly().CreateInstance(typeof(T).FullName)));
        }
        ...
        /// Executes an update query for the specified entity.
        /// </summary>
        public static bool Update<T>(this IDbConnection connection, IEnumerable<T> entities, int? commandTimeout = null) where T : class
        {
            bool res = false;
            using (IDbTransaction transaction = connection.BeginTransaction())
            {
                ...
                {
                    Instance.Update<T>(connection, entity, transaction, commandTimeout);
                }
                try
                {
                    transaction.Commit();
        ...
        /// Executes a delete query for the specified entity.
        /// </summary>
        public static bool Delete<T>(this IDbConnection connection, IEnumerable<T> entities, int? commandTimeout = null) where T : class
        {
            bool res = false;
            using (IDbTransaction transaction = connection.BeginTransaction())
            {
                foreach (var entity in entities)
                {
                    Instance.Delete<T>(connection, entity, transaction, commandTimeout);
                }
                try
                {
                    transaction.Commit();
